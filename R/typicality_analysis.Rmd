...
ngram <- typicality_ratings %>% 
  filter(model == "5gram") %>% 
  rename(ngram_lp = logprob, ngram_nlp = normalized_logprob) %>% 
...
ngram_fo <- feature_overlaps %>% 
  filter(model == "5gram") %>% 
  rename(ngram_f_lp = logprob, ngram_f_nlp = normalized_logprob) %>% 
  select(-stimulus, -model, -params)
```
...
typicality_ratings %>%
  ...
  nest() %>%
  mutate(
    cor = map(data, function(x) cor.test(x$ngram_lp, x$logprob, method = "kendall") %>% tidy())
  ) %>%
  unnest(cor) %>%
  select(-data) %>%
...
ngram %>%
  group_by(category) %>%
  nest() %>%
  mutate(cor = map(data, function(x) cor.test(x$rank, x$ngram_lp, method = "kendall") %>% tidy())) %>%
  unnest(cor) %>%
  select(-data)
```
...
feature_overlaps %>%
  ...
  nest() %>%
  mutate(
    cor = map(data, function(x) cor.test(x$ngram_f_lp, x$logprob, method = "kendall") %>% tidy())
  ) %>%
  unnest(cor) %>%
  select(-data) %>%
  group_by(model, params) %>%
